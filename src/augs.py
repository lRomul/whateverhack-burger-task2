...
class HorizontalFlip:
    ...
    def __call__(self, img, mask=None):
        # print('in HorizontalFlip')
        if random.random() < self.prob:
            # print('working HorizontalFlip')
            img = cv2.flip(img, 1)
            if mask is not None:
...
class RandomBrightness:
    ...
    def __call__(self, img):
        # print('in RandomBrightness')
        if random.random() < self.prob:
            # print('working RandomBrightness')
...
class RandomHueSaturationValue:
    ...
    def __call__(self, image):
        # print('in RandomHueSaturationValue')
        if random.random() < self.prob:
            # print('working RandomHueSaturationValue')
...
def augment(x, mask=None, prob=0.5):
    return DualCompose([
        OneOrOther(*(
            ...
            ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.10, rotate_limit=10)
        ), prob=prob),
        HorizontalFlip(prob=0.5),
        ImageOnly(RandomContrast(limit=0.1, prob=0.5)),
        ImageOnly(RandomFilter(limit=0.2, prob=0.2)),
    ])(x, mask)


def augment_a_little(x, mask=None):
    return DualCompose([
